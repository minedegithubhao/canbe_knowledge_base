...
- **问题记录**
	...
	- 2026-05-26 全库收口复查结论：
		- 当前 `pages/` 下 Markdown 页面约 548 个。
		- 标准 namespace 或核心入口页面约 355 个。
		- 旧命名或非标准命名页面约 193 个，其中大多数已经有迁移说明，可作为来源或归档页保留。
		- 本轮处理项：
			- 将 [[MOC/向量检索]]、[[MOC/大模型应用工程]]、[[MOC/数据库与数据同步]] 中的“尚未整理”改为“后续深挖”，避免把进阶主题误判为结构重构未完成。
			- 为 [[知识库维护规则]] 补齐更新时间。
			- 同步更新 [[首页]] 和本项目页的更新时间。
		- 可保留观察项：
			- 大量旧概念页虽然仍是旧文件名，但已有迁移说明和新页面承接，不以删除为目标。
			- 旧归档页缺少 `updated` 不影响当前阅读路径，后续可按主题小步补齐。
...
- **批次记录**
	...
	- 2026-05-26
		- 完成发布预演复查：实际运行 `scripts/prepare-publish-graph.ps1` 并生成 `.publish-graph`。
		- 发布输出只包含 `assets/`、`logseq/` 和 `pages/`；`journals/` 与 `whiteboards/` 已按默认规则排除。
		- 发布输出包含 548 个页面、33 个已引用图片附件和必要 Logseq 配置；7 个未引用附件被预演脚本从输出目录移除。
		- 精确凭据扫描未发现 API key、密码、Authorization、Bearer token、数据库连接串或云访问密钥。
		- private 关键词命中主要来自维护规则说明、Python 私有属性示例和 RAG 私有知识概念说明；未发现真实 private 标记泄漏。
	- 2026-05-26
		- 完成旧页抽样验收：按 RAG、Python、机器学习、数据库、大模型和 Transformer 抽查旧命名页面。
		- 抽查样本包括 [[大模型/RAG]]、[[大模型/RAG/Query Rewriting]]、[[Python/函数]]、[[Mysql/表关联]]、[[机器学习分类模型评估方法]]、[[AI大模型发展方向]]、[[项目实战]] 和 [[面试背诵汇总]]。
		- 验收结论：除 [[excalidraw-library-items-storage]] 作为插件数据页保留外，旧命名页面均已具备迁移说明、归档说明或来源保留说明。
		- 旧页继续保留为历史资料和反向链接入口，不以删除旧文件为目标。
	- 2026-05-26
		- 完成入口路径压测：检查 [[首页]]、[[MOC/面试复习]]、[[MOC/RAG]]、[[MOC/Python]]、[[MOC/机器学习]]、[[Review/RAG面试]]、[[Review/Python面试]] 和 [[Review/机器学习面试]]。
		- 压测路径覆盖：首页到主题 MOC、首页到 Review、MOC 到 Concept、MOC 到 Question、Review 到 Question、Review 到关联 Concept。
		- 链接解析结果：上述 8 个入口页面共 757 个 wikilink 均能解析到现有页面或旧页承接入口。
		- 同步更新被压测的 MOC 和 Review 页面更新时间。
	- 2026-05-26
		- 完成 Review 题单质量抽查：重点检查 [[Review/RAG面试]]、[[Review/Python面试]] 和 [[Review/机器学习面试]]。
		- 验收结论：三者都具备刷题顺序、高频卡片、复杂题入口和关联概念；RAG 与机器学习已有 Mistake 承接。
		- 修补结果：为 [[Review/Python面试]] 补充 `- **易错题**` 区块，覆盖元组可变元素、可变默认参数、`is` 与 `==`、异常捕获和生成器使用边界。
	- 2026-05-26
		- 完成项目面试链路补强：检查 [[Project/手册类RAG实战]]、[[Project/智能客服RAG实战]]、[[Project/图文知识库]] 和 [[Review/RAG项目面试]]。
		- 修补结果：为 [[Project/手册类RAG实战]] 补充 `- **指标和验收**`，让项目能讲清证据命中、Top K 纯度、真实性、相关性和参数收益。
		- 修补结果：为 [[Project/图文知识库]] 补充 `- **排查和取舍**`，让图文项目能讲清 OCR、caption、metadata、融合排序和视觉模型成本之间的取舍。
		- 修补结果：为 [[Review/RAG项目面试]] 补充 `- **项目易错点**`，覆盖“只讲向量库”“只报最终准确率”和“不讲副作用”三类常见面试问题。
	- 2026-05-26
		- 完成证据链抽查：检查 [[Concept/RAG]]、[[Concept/混合检索]]、[[Concept/重排序]]、[[Concept/模型评估]] 和 [[Concept/Python操作MySQL]]。
		- 验收结论：5 个 Concept 均已有 `- **来源**` 区块；RAG、混合检索和重排序的来源链能回到旧页和项目页。
		- 修补结果：为 [[Concept/模型评估]] 补充 RAG 评估项目链路来源，避免只停留在分类指标旧笔记。
		- 修补结果：为 [[Concept/Python操作MySQL]] 补充 `- **证据链说明**`，明确旧页只支撑主题入口，工程要点需结合对应 Question 和 MySQL 相关概念。
	- 2026-05-26
		- 完成最终发布前回归检查：重新运行 `scripts/prepare-publish-graph.ps1`，生成 `.publish-graph`。
		- 发布输出：默认排除 `journals/` 和 `whiteboards/`，保留 548 个页面、33 个已引用附件，并从输出目录清理 7 个未引用附件。
		- 风险扫描：未发现 private 标记、API key、密码、Authorization、Bearer token、数据库连接串或云访问密钥泄漏。
		- 本地和内网路径扫描：仅命中 [[评估案例Prompt]] 中 Python REPL 示例的 `>>>` 字符串，判定为误报。
		- 主入口链接检查：[[首页]]、[[MOC/面试复习]]、[[MOC/RAG]]、[[Review/RAG面试]]、[[Review/RAG项目面试]]、[[Project/手册类RAG实战]]、[[Concept/RAG]] 共 396 个 wikilink，断链数为 0。
		- 最终结论：当前知识库达到“可发布、可长期维护”的状态；后续进入按资料和主题小步维护。
	- 2026-05-03
		- 完成后续质量提升任务总收口。
	...
	- 2026-05-03
		...
		- 2026-05-26 复查时已实际运行 `scripts/prepare-publish-graph.ps1`，确认默认发布输出会排除 journals 和 whiteboards，并清理未引用附件。
