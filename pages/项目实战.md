...
- **迁移说明**
	- 本页面是旧项目面试素材，稳定内容已整理到 [[Review/RAG项目面试]]、[[Review/RAG面试]] 和相关 Project 页面。
	- 后续项目刷题优先维护 [[Review/RAG项目面试]]；本页保留为历史资料。

- **原始信息**
	- 旧项目面试背诵素材，包含向量库构建、chunk 去重、结构化输出和 RAG 工程追问。

- **核心观点**
	- 项目面试要讲清楚数据入库、检索、重排、生成、评估和上线风险。
	- RAG 工程不仅是模型调用，还包括数据去重、结构化输出、日志、评估和回归。
	- 复杂答案应沉淀到 Project 和 Question，Review 页面只保留刷题入口。
...
- **待追问问题**
	- RAG 项目面试中如何用一条主线讲清楚“问题、定位、方案、指标、结果”？
